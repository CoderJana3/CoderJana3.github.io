    ...
    return t.getRestApi()                                                                       //get a REST API Instance
            ...
            .then(function(authorized){ //dont put authorized => function, nothing happens literally nothing not even what is supposed to 
                if(authorized){                                                                 //If the User is authorized set isAuth to true
                    ...
                    return t.popup({                                                            //and open a Popup to further save the attachment
                        title: 'Trello is Authorized',
                        url: './tautht.html'
                    })
                } else {
                    isAuth = false;                                                             //If the User is not authorized set isAuth to false
                    console.log("User is authorized: " + isAuth);
                    return t.popup({                                                            //and open a Popup to start the authorization 
...
async function getEnv() {
   const envvar = await fetch("/.netlify/functions/envvar")                                     //need this to get environemnt vars from netlify 
    ...
    const envkey = envvar.apikey;                                                               //save the apikey in var envkey to use it later on
                                                                                                //as the apikey is already stringified in the Body 
                                                                                                //of the response there is no need to use stringify again
   
   window.TrelloPowerUp.initialize({                                                            //Start the Initialisation needed for all Capabilities
    'card-buttons': function(t, options){                                                       //Define the Card Buttons to use
        //return an array at card-badges 
        return [{
            
            //icon: needs to be a specific format
            text: 'Estimate Size',                                                              //Card Button from the Tutorial
            callback: function(t){
                return t.popup({
                    title: "Estimation",
                    url: "estimate.html",
    ...
    'card-badges': function(t, options){                                                        //Card Badges from the Tutorial
    ...
    'card-detail-badges': function(t, options){                                                 //Card Detail Badges from the Tutorial                                              
        return t.get("card", "shared", "estimate")
        .then(function(estimate){
     ...
     'authorization-status': function(t, options){                                              //Authorization Status from the Tutorial for a 3rd Party System
     ...
     'show-authorization': function(t, options){                                                //Show Authorization Status from the Tutorial for a 3rd Party System
     ...
     'save-attachment': function(t, options){
        ...
        return [{                                                                               //if you put multiple callbacks, the lowest one will be carried out
            callback: isAuthorizedTest,                                                         //Check to see if a User is authorized before saving an attachment
             },
        ]                                  
    },
    },
    {   
        appKey: envkey,                                                                         //Define necessary Information for the REST API Instance
...
getEnv();                                                                                       //Needed for all other Functions to work 
